    ...
    const int flag, double* osbd)
{
    ...
    if (flag < 0 || flag >= gs*gs) // 从0开始计数
    {
        cerr << "flag is out of range" << endl;
    }
    if (flag % gs != 0 && flag % gs != order && flag >= gs && flag < gs*gs-gs) 
    // flag在中心，边界值为全0
    {
        for (int i = 0; i < 2*(nx*order+1)+2*(ny*order+1); i++)
        {
            osbd[i] = 0.0;
        }
        cout << "osbd is all zero, are you sure?" << endl;
    }
    else{
    const int gs = order + 1;
    ...
    for (int i = 0; i < gs*ny; i++) zerocol[i] = 0.0;
    for (int i = 0; i < gs*nx; i++) zerorow[i] = 0.0;
    ...
    if (flag == 0)
    {
        ...
        for (int i = ny*order+1; i < ny*order + 2*nx*order - 1; i++)
        {
            ...
            if (index%2==0) osbd[i] = e2[index/2];
            else osbd[i] = 0.0;
        }
    ...
    else if (flag == gs-1)
    {
        for (int i = 0; i < ny*order+1; i++)
        {
            osbd[i] = e1[i + order*(ny*order+1)];
        }
        for (int i = ny*order+1; i < ny*order + 2*nx*order - 1; i++)
        {
            int index = i - (ny*order+1) + 1;
            if (index%2==0) osbd[i] = e3[index/2];
            else osbd[i] = 0.0;
        }
        for (int i = ny*order + 2*nx*order - 1; i < sizebd; i++)
        {
    ...
    else if (flag == gs*gs-gs)
    {
        ...
        {
            int index = i - (ny*order+1) + 2;
            if (index%2==0) osbd[i] = e2[order*(nx*order+1) + index/2];
            else osbd[i] = 0.0;
        }
        for (int i = ny*order + 2*nx*order - 1; i < sizebd; i++)
        {
    ...
    else if (flag == gs*gs-1)
    {
        ...
        {
            int index = i - (ny*order+1) + 1;
            if (index%2==0) osbd[i] = e3[order*(nx*order+1) + index/2];
            else osbd[i] = 0.0;
        }
        for (int i = ny*order + 2*nx*order - 1; i < sizebd; i++)
        {
            int index = i - (ny*order + 2*nx*order - 1);
            osbd[i] = e4[order*(ny*order+1) + index];
        }
    }
    ...
    else if (flag > gs-1 && flag < gs*gs-gs-1 && flag % gs == 0)
    {
        ...
        for (int i = ny*order+1; i < ny*order + 2*nx*order - 1; i++)
        {
            ...
            if (index%2==0) osbd[i] = e2[bind*(nx*order+1) + index/2];
            else osbd[i] = 0.0;
        }
        for (int i = ny*order + 2*nx*order - 1; i < sizebd; i++)
        {
    ...
    else if (flag > gs && flag < gs*gs-1 && flag % gs == order)
    {
        ...
        for (int i = ny*order+1; i < ny*order + 2*nx*order - 1; i++)
        {
            int index = i - (ny*order+1) + 1;
            int bind = (int)flag/gs;
            if (index%2==0) osbd[i] = e3[bind*(nx*order+1) + index/2];
            else osbd[i] = 0.0;
        }
        for (int i = ny*order + 2*nx*order - 1; i < sizebd; i++)
        {
    ...
    else if (flag > gs*gs-gs && flag < gs*gs-1)
    {
        ...
        {
            int index = i - (ny*order + 2*nx*order - 1);
            osbd[i] = e4[(flag-gs*order)*(ny*order+1) + index];
        }
    }
    delete [] e1;
    delete [] e2;
